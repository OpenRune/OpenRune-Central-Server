#!/usr/bin/env bash
# Sync Pterodactyl scripts when the GitHub branch tip commit changes (public repo API + raw files).

AUTOUPDATE_STATUS="${AUTOUPDATE_STATUS:-1}"
AUTOUPDATE_FORCE="${AUTOUPDATE_FORCE:-1}"
AUTOUPDATE_REPO="${AUTOUPDATE_REPO:-OpenRune/OpenRune-Central-Server}"
...
fetch_remote_commit_sha() {
    ...
    # First 40-char hex in the response is the branch-tip commit SHA.
...
if ! enabled "${AUTOUPDATE_FORCE}"; then
    echo -e "${CYAN}[AutoUpdate] Set AUTOUPDATE_FORCE=1 to download scripts automatically.${NC}"
    exit 0
fi

# Pin all downloads to this commit so every file matches the same revision.
...
echo -e "${WHITE}[AutoUpdate] Downloading scripts from commit $(short_sha "${REMOTE}")...${NC}"
...
echo -e "${CYAN}[AutoUpdate] Re-import the egg in the panel when new variables are added.${NC}"
