...
class CentralActivityLogRepository(
        ...
        return dataSource.connection.use { conn ->
            conn.prepareStatement(sql).use { ps ->
                ...
                ps.executeQuery().use { rs ->
                    if (!rs.next()) null
                    else rs.toCentralActivityLogRow()
                }
            }
        }
    }
    ...
    fun countDistinctLoginAccountsForDay(dayUtc: String): Int {
        ...
        return dataSource.connection.use { conn ->
            conn.prepareStatement(sql).use { ps ->
                ...
                ps.executeQuery().use { rs ->
                    require(rs.next()) { "COUNT query produced no row" }
                    rs.getInt(1)
                }
            }
        }
    ...
    fun insert(log: CentralActivityLog): UUID {
        ...
        val payload = PGobject().apply {
            type = "jsonb"
            value = CentralActivityLogJson.json.encodeToString(
                CentralActivityLog.serializer(),
                log
            )
        }
        ...
        return dataSource.connection.use { conn ->
            ...
            try {
                ...
                val (logId, uuid) = conn.prepareStatement(sql).use { ps ->
                ...
                val itemLines =
                    (log as? ItemLineProducer)?.itemLines().orEmpty()

                insertActivityLogItems(conn, logId, itemLines)
    ...
    private fun ResultSet.toCentralActivityLogRow(): CentralActivityLogRow =
        CentralActivityLogRow(
            id = getLong("id"),
            logUuid =
                getObject("log_uuid", UUID::class.java)
                    ?: error("log_uuid was null"),
